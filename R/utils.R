validate_auth_info <- function(api_key, client_id, client_secret) {
  if (anyNA(c(api_key, client_id, client_secret))) {
    stop("`api_key`, `client_id`, and`client_secret` must be supplied as
...
get_auth_vars <- function(api_key = NULL,
                          client_id = NULL,
                          client_secret = NULL) {
  ...
  if (is.null(client_secret)) {
    client_secret <- Sys.getenv("ENVERUS_CLIENT_SECRET")
  }
...
has_link <- function(response) {
  if (length(response$headers$links) > 0) {
    if (stringr::str_detect(response$header$links, "next")) {
      return(TRUE)
    }
  }
  return(FALSE)
